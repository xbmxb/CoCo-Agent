...
def fetch_episode(dataset_name, data_split, get_images, get_annotations, get_actions, output_dir):
    ...
    episode_id = None
    numi = 0
    for d in tqdm(dataset, total=490360):
        numi+=1
        if numi > 490360:
            break
        ex = tf.train.Example()
...
if __name__ == '__main__':
    ...
    with open(f"{args.output_dir}_pre10_train.obj", "wb") as wp:
        pickle.dump(all_parsed_episode["train"],wp)
    with open(f"{args.output_dir}_pre10_val.obj", "wb") as wp:
        pickle.dump(all_parsed_episode["val"],wp)
    with open(f"{args.output_dir}_pre10_test.obj", "wb") as wp:
        pickle.dump(all_parsed_episode["test"],wp)

# python fetch_dataset_for_t5_blipv2_google_pre10.py --split_file "dataset/splits/standard.json" --output_dir "dataset/owl/google_apps_parsed_episode_owl_pre10" --dataset google_apps
