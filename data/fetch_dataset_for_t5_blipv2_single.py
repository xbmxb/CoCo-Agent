...
def parse_episode(
    ...
    for i, ex in enumerate(episode):
        ...
        if get_images:
            ...
            # dir_name = ep_id.split('.')[0]
            image_path = os.path.join(output_dir, str(ep_id.split('.')[0]) +'_'+str(ep_id.split('.')[1]) +'/'+str(step_id)+'.png')
            if not os.path.exists(os.path.join(output_dir, str(ep_id.split('.')[0]) +'_'+str(ep_id.split('.')[1]))):
                os.mkdir(os.path.join(output_dir, str(ep_id.split('.')[0]) +'_'+str(ep_id.split('.')[1])))
    ...
    for d in tqdm(dataset):
        ...
        if episode_id is None:
            ...
            try:
                ...
                episode = [ex]
            # print(episode_id)
            if str(episode_id) in train_data:
                curr_split = "train"
            elif str(episode_id) in val_data:
                curr_split = "val"
            elif str(episode_id) in test_data:
                curr_split = "test"
            else:
                print("error episode")
                print(episode_id)
...
# python fetch_dataset_for_t5_blipv2_single.py --split_file "dataset/splits/zzs_splits/single_split.json" --output_dir "dataset/owl/fix_single_parsed_episode_owl" --dataset single ---------failed, the episode id is not inttt
