...
def eval_for_metrics(args):
    ...
    for idx, pred in enumerate(preds):
        try:
            ...
            reference_cot = targets_cot[idx]['ground_truth']
        except:
        ...
        try:
            if '<STATUS_TASK_COMPLETE>' in preds[idx]:
                ...
                # need refine 
                if '<CLICK>' in preds[idx]:
                    # the icon name
                    # I need to <CLICK> Chile | Today's latest from Al, the location of
                    icon = preds[idx].split('<CLICK> ')[-1].split(', the location of')[0]
                    icon_ref = reference_cot.split('<CLICK> ')[-1].split(', the location of')[0]
                    # fetch the layout
                    layouts = targets_cot[idx]['prompt'].split('<image>\n')[-1].split('\n\nPrevious Actions')[0].split('\n')
                    point = tap_point
                    for lo in layouts:
                        if icon.strip() == lo.split("location:")[0].strip():
                            point = lo.split("location: ")[-1].strip()
                            break
                
                if '<CLICK>' in preds[idx] and icon + ' location: '+tap_point not in layouts and point != tap_point:
                    print('refine <CLICK> +1')
                    print(preds[idx])
                    print(targets_cot[idx]['ground_truth'])
                    print(icon, tap_point, lo, point)
                    tap_point = point
                ...
                # need refine
                if 'up' in preds[idx]:
                    tp1, lp1 =  scroll_map['up']
                elif 'down' in preds[idx]:
                    tp1, lp1 = scroll_map['down']
                elif 'right' in preds[idx]:
                    tp1, lp1 =  scroll_map['right']
                elif 'left' in preds[idx]:
                    tp1, lp1= scroll_map['left']
                tp = eval(preds[idx].split('"touch_point": ')[1].split(', "lift_point": ')[0].strip())
                lp = eval(preds[idx].split(', "lift_point": ')[1].strip())
                if not (tp==str(tp1) and lp==str(lp1)):
                    print(type(tp), lp, type(tp1), lp1)
                    tp, lp = str(tp1), str(lp1)
                    print('refine <SCROLL> + 1')
                    # os._exit(0)
                pred = eval('{ "action_type": "DUAL_POINT", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                pred['touch_point'] = tp
                pred['lift_point'] = lp
...
if __name__ == "__main__":
    ...
    parser.add_argument("--answers-file", type=str, default="/data/maxb/tag/LLaVA/llava/eval/res_cot_out/cotv0_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl")
    # parser.add_argument("--conv-mode", type=str, default="llava_llama_2")
    # parser.add_argument("--num-chunks", type=int, default=1)
    # parser.add_argument("--chunk-idx", type=int, default=0)
    # parser.add_argument("--answer-prompter", action="store_true")
    parser.add_argument("--prd_output_path", type=str, default='/data/maxb/tag/LLaVA/llava/eval/res_cot_out')
    parser.add_argument("--eval_name", type=str, default='cotv0_8histlocationtotal_scores_plus_plus')
...
# python eval_aitw_cot_plus.py --answers-file ./res_cot_out/cotv0_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl  --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_cot_out --eval_name cotv0_8histlocationtotal_scores_plus_plus
...
# python eval_aitw_cot_plus.py --answers-file ./res_cot_out/cotv0_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl  --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_cot_out --eval_name cotv0_8histlocationtotal_scores_plus_plus
# python eval_aitw_cot_plus.py --answers-file ./res_continue/conv2_total_t0t0_cotv0_8histlocation6k_llava_try-llama-2-7b-chat-finetune.jsonl  --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_continue --eval_name conv2_t0t0_total_scores_plus