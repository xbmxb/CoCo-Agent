...
def eval_for_metrics(args):
    ...
    for idx, pred in enumerate(preds):
        try:
            ...
            continue
        ############check the prediction for the corruption##############
        mt = eval('{"Correct image"' + preds[idx].split('"Correct image"')[-1] + '}')
        preds[idx] = preds[idx].split(', "Correct image"')[0]
        mtr = eval('{"Correct image"' + reference_cot.split('"Correct image"')[-1] + '}')
        reference_cot = reference_cot.split(', "Correct image"')[0]
        mt_ = [ 1 if mt[ic] else 0 for ic in mt.keys() ]
        mtr_ = [ 1 if mtr[ic] else 0 for ic in mtr.keys() ]
        if mt_ == mtr_:
            mt_correct += 1
            mt_label = 1
        else:
        ...
        action_data = {"pred": action_1_wrap, "target": action_2_wrap, "match_label": match_label, "multi_task": (mtr_, mt_, mt_label) }
    ...
    metrics["mt_acc"] = "{:.2f}".format(mt_correct/len(targets) * 100)
...
# multitask
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep1_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep1
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep2_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep2
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep3_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep3
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep4_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep4
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep5_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep5
# python eval_aitw_mt.py --answers-file ./res_multitask/v0_ep6_8histlocationtotal_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path ./res_multitask --eval_name mtv0_ep6