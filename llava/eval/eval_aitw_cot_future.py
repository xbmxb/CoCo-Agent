...
def eval_for_metrics(args):
    ...
    for idx, pred in enumerate(preds):
        ...
        if '\nNext action:' in preds[idx]:
            preds_idxes = preds[idx].split('\nNext action:')
            reference_cot_s = reference_cot.split('\nNext action:')
            if not len(preds_idxes) == len(reference_cot_s):
                # print(preds_idxes)
                # print(reference_cot_s)
                # os._exit(0)
                reference_cot_s = reference_cot_s[:min(len(reference_cot_s), len(preds_idxes))]
                preds_idxes = preds_idxes[:min(len(reference_cot_s), len(preds_idxes))]
        for ts in range(len(preds_idxes)):
            if idx+ts >= len(targets):
                break
            reference = eval("{" + targets[idx+ts] + "}")
            print('preds_idxes: ', preds_idxes[ts])
            print('reference_cot_s: ', reference_cot_s[ts])
            cot1 = '<' + preds_idxes[ts].split('<')[1].split(">")[0] + '>'
            cot2 ='<' + reference_cot_s[ts].split('<')[1].split(">")[0] + '>'
            if cot1 == cot2:
                cot_acc += 1
            else:
                print('not match in cot action: ', cot1, cot2, reference_cot_s[ts])
                if cot1 not in cot_type_f.keys():
                    cot1 = '<STATUS_TASK_IMPOSSIBLE>'
                
            # parsing for CoT
            try:
                if '<STATUS_TASK_COMPLETE>' in preds_idxes[ts]:
                    pred = eval('{ "action_type": "STATUS_TASK_COMPLETE", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                elif '<PRESS_BACK>' in preds_idxes[ts]:
                    pred = eval('{ "action_type": "PRESS_BACK", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                elif '<PRESS_HOME>' in preds_idxes[ts]:
                    pred = eval('{ "action_type": "PRESS_HOME", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                elif '<PRESS_ENTER>' in preds_idxes[ts]:
                    pred = eval('{ "action_type": "PRESS_ENTER", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                elif '<TYPE>' in preds_idxes[ts]:
                    pred_type_text = preds_idxes[ts].split('"typed_text":')[1].strip()
                ...
                elif '<TAP>' in preds_idxes[ts] or '<CLICK>' in preds_idxes[ts]:
                    tap_point = eval(preds_idxes[ts].split('"tap_point":')[1].strip())
                ...
                elif '<SCROLL>' in preds_idxes[ts]:
                    tp = eval(preds_idxes[ts].split('"touch_point": ')[1].split(', "lift_point": ')[0].strip())
                    lp = eval(preds_idxes[ts].split(', "lift_point": ')[1].strip())
                    pred = eval('{ "action_type": "DUAL_POINT", "touch_point": "[-1.0, -1.0]", "lift_point": "[-1.0, -1.0]", "typed_text": ""}')
                    pred['touch_point'] = tp
                    pred['lift_point'] = lp
                else:
            ...
            annotation_positions = reference_test_positions[idx+ts]
            total_num += 1
            try:
    ...
    metrics["cot_acc"] = "{:.2f}".format(cot_acc/total_num * 100)
    metrics["partial_acc"] = "{:.2f}".format(partial_correct/total_num * 100)
    metrics["text_acc"] = "{:.2f}".format(text_correct/total_num * 100)
    metrics["type_acc"] = "{:.2f}".format(type_correct/total_num * 100)
    metrics["partial_correct"] = partial_correct
    metrics["text_correct"] = text_correct
    metrics["type_correct"] = type_correct
    metrics["total_num"] = total_num
    print(metrics)
    output_data_dic = {
        "metrics": metrics,
        "data": output_data
...
# python eval_aitw_cot_future.py --answers-file /data/maxb/tag/LLaVA/llava/eval/res_future/6k_future2_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_future --eval_name 6k
# python eval_aitw_cot_future.py --answers-file /data/maxb/tag/LLaVA/llava/eval/res_future/9k_future2_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_future --eval_name 9k_future
# python eval_aitw_cot_future.py --answers-file /data/maxb/tag/LLaVA/llava/eval/res_future/12k_future2_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_future --eval_name 12k_future
# python eval_aitw_cot_future.py --answers-file /data/maxb/tag/LLaVA/llava/eval/res_future/total_future2_llava_try-llama-2-7b-chat-finetune.jsonl --prd_output_path /data/maxb/tag/LLaVA/llava/eval/res_future --eval_name total_future