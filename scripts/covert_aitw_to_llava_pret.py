import os, json
from utils_data_for_owl_pret import load_for_owl
...
def convert_to_llava(base_dir, split, prompt_format="QCM-LEA", name=''):
    ...
    outputfile = os.path.join('./aitw_data/pret/', f"llava_aitwfull{name}_{split}_{prompt_format}.json")
    if os.path.exists(outputfile):
        raise FileExistsError
    with open(outputfile, "w") as f:
        json.dump(target_format, f, indent=2)

convert_to_llava('.', 'test', name='_pretrainv1')