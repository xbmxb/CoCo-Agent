...
def load_data(args, split):
    ...
    for qid, episode in enumerate(tqdm(data)):
        ...
        for step_idx, step_data in enumerate(episode_data):
            ...
            if args.use_history:
                ...
                prev_actions_all.append(prev_actions)
                if args.use_img_history:
    ...
    cid_f.close()
    center_f.close()
    return source_text, source_image, target_text, anno_positions, goals, element_ids, element_texts, icon_strings
...
def load_for_owl(inputfile, split, foreval=False, margs = None):
    ...
    print("args",args)
    source_text, source_image, target_text, anno_positions, goals, element_ids, element_texts, icon_strings = load_data(args, split)
