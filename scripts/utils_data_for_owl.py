...
def load_data(args, split):
    ...
    if args.all_data:
        if split == "train":
            ...
            for subdir in ["general", "google_apps", "install", "single", "web_shopping"]:
                ...
                subdir_ = subdir+args.data_root
                if subdir == "google_apps":
                    subdir_  = subdir+args.data_root+'_pre10_pre10'
                with open(f"{args.data_path}/{subdir_}_{split}.obj", "rb") as rp:
    ...
    cid_f.close()
    center_f.close()
    return source_text, source_image, target_text, anno_positions, goals
...
def load_for_owl(inputfile, split, foreval=False, margs = None):
    ...
    print("args",args)
    source_text, source_image, target_text, anno_positions, goals = load_data(args, split)
